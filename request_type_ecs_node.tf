resource "aws_spot_fleet_request" "ecs_nodes_no_lb" {
  count               = "${ var.lb_integration ? 0 : "${var.type == "ecs_node" ? 1 : 0}" }"
...
resource "aws_spot_fleet_request" "ecs_nodes_lb" {
  count               = "${ var.lb_integration ? "${var.type == "ecs_node" ? 1 : 0}" : 0 }"
  iam_fleet_role      = "${aws_iam_role.spot-fleet-tagging-role.arn}"
  allocation_strategy = "diversified"
  target_capacity     = "${var.capacity}"
  valid_until         = "${var.valid_until}"
